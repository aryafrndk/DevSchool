...
$stmt = mysqli_prepare($koneksi, "SELECT * FROM user WHERE role = 3");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
    ...
    $stmt = mysqli_prepare($koneksi, "UPDATE user SET nama = ?, email = ?, alamat = ?, jabatan = ? WHERE id = ?");
    mysqli_stmt_bind_param($stmt, 'ssssi', $nama, $email, $alamat, $jabatan, $id);
    $success = mysqli_stmt_execute($stmt);
                ...
                <tbody>
                    <?php while ($user = mysqli_fetch_assoc($result)): ?>
                        <tr>
                            ...
                            <td><?php if ($user['role'] == 3) { echo 'Tenaga Pelatih';} ?></td>
                            ...
                            <td>
                                ...
                                data-jabatan="<?php echo htmlspecialchars($user['jabatan']); ?>">Edit</button>
    ...
    button.addEventListener('click', function() {
        ...
        Swal.fire({
            ...
            html:
                ...
                `<input id="jabatan" class="swal2-input" placeholder="Jabatan" value="${jabatan}">`,
            showCancelButton: true,
...
<script>
    ...
    document.querySelectorAll('.btn-view').forEach(button => {
        button.addEventListener('click', function () {
            const userId = this.getAttribute('data-id'); // Ambil ID user
            ...
            fetch(`?view_id=${userId}`)
                ...
                .then(data => {
                    if (data.error) {
                        ...
                        // Jika berhasil, tampilkan data user dalam SweetAlert
                        Swal.fire({
                            title: 'User Data',
                            html: `
                ...
                .catch(error => {
                    // Jika terjadi error saat AJAX request
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
